...
/*
 * populateMsgHdr
 *	populate the IFW message header with the passed in information.
 *
 * Parameters:      rsocket - Socket file descriptor
 *                  msg_hdr - pointer to the message header.
 *                  msg_type - type of message
 *                  msg_size - message size
 *                  defw_version_number - version number
 *
 */
...
defw_rc_t readTcpMessage(int iFd, char *pcBuffer,
			 int iBufferSize, int iTimeout, bool force_wait)
{
	size_t tNleft;
	ssize_t tNread;
	char *pcCur;
